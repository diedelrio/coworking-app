...
async function getReservationRules() {
  ...
  const closeHour = toTyped(m.OFFICE_CLOSE_HOUR?.value, m.OFFICE_CLOSE_HOUR?.valueType, 20);
  ...
  // DIEGO console.log('Loaded reservation rules from settings:', { openHour, closeHour, minMinutes, stepMinutes, minMinutesBefore });
  return {

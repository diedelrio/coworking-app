...
export default function OperationsBulkTokenRegen() {
  ...
  const [tokenType, setTokenType] = useState('ACTIVATION');
  ...
  const execute = async () => {
    ...
    try {
      ...
      const res = await api.post('/admin/operations/bulk-token-regenerate/execute', {
        tokenType,
        templateKey,
        audience,
      });
  ...
  return (
    <div>
      <h2 style={{ marginTop: 0 }}>Regenerar tokens y reenviar</h2>
      <p style={{ marginTop: 0, color: '#6b7280' }}>
        Regenera tokens de <b>activación</b> o <b>reset</b> y envía un template por <b>key</b> al segmento elegido.
      </p>
      ...
      <div style={{ display: 'grid', gap: '0.75rem', maxWidth: 720 }}>
        <div style={{ display: 'grid', gap: '0.25rem' }}>
          ...
          >
            <option value="ACTIVATION">ACTIVATION (activar cuenta)</option>
            <option value="RESET">RESET (olvidé contraseña)</option>
          </select>
        </div>
        ...
        >
          {loadingRun ? 'Ejecutando…' : 'Regenerar y enviar'}
        </button>

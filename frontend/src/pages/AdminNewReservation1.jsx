...
export default function AdminNewReservation() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // espacios activos
        const spacesRes = await api.get('/spaces/active');
        // listado de usuarios para el combo (crearemos/ajustaremos el endpoint si hace falta)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // volvemos al dashboard admin o donde prefieras
      navigate('/admin');
    } catch (err) {
      console.error(err);
      setError(
  ...
  if (!currentUser || currentUser.role !== 'ADMIN') {
    // seguridad extra por si alguien llega directo
    navigate('/login');
    return null;
  }
